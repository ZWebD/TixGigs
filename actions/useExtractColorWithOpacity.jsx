...
const defaultOptions = {
  ...
  format: "rgba",
  maxSize: 10,
  opacity: 1, // Changing the opacity of the desired color only RGBA,HSLA,
};

// If the picture has alpha value then don't need the default value???
    ...
    async function fetchDominantColor() {
      try {
        ...
        if (isMounted) {
          ...
          setOpacity(formattedColors.opacity);
        }
        console.log(opacity);
      } catch (error) {
        if (isMounted) {
...
function formatColors(colors, format, maxColors, opacity) {
  switch (format) {
    case "hex":
      return {
...
// functions para converting colors
...
function rgbaToHsl(rgba) {
  ...
  const a = parseInt(match[4]); // alpha component
